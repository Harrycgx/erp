...
import { fetchCustomerQuotationDetail, approveCustomerQuotation, rejectCustomerQuotation, requestCustomerQuotationRevision } from '../services/customerPortalService';
...
export default function CustomerQuotationDetails() {
  ...
  const loadQuotation = async () => {
    setLoading(true);
    setError('');
    const { data, error: loadError } = await fetchCustomerQuotationDetail(quotationId);
    if (loadError) {
  ...
  const handleSubmitAction = async () => {
    ...
    if (modalAction === 'approve') {
      response = await approveCustomerQuotation({ quotation, note, actorId: user?.id });
    } else if (modalAction === 'reject') {
      response = await rejectCustomerQuotation({ quotation, note, actorId: user?.id });
    } else {
      response = await requestCustomerQuotationRevision({ quotation, note, actorId: user?.id });
    }
