...
export default function ProductionTable({ jobs, onSelectJob, linkToDetail = false }) {
  return (
    <div className="overflow-hidden rounded-[32px] border border-slate-700 bg-slate-950/95 shadow-xl shadow-black/20">
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-slate-800 text-sm text-slate-300">
          <thead>
            <tr>
              <th className="px-5 py-4 text-left text-xs uppercase tracking-[0.3em] text-slate-500">Job / SO</th>
          ...
          <tbody className="divide-y divide-slate-800">
            {jobs.map((job) => (
              <tr key={job.id} className={`hover:bg-slate-900/80 ${isOverdue(job) ? 'bg-amber-950/20' : ''}`}>
                <td className="px-5 py-4">
                  <p className="font-semibold text-white">{job.production_number || 'PRD'}</p>
                  <p className="mt-1 text-xs text-slate-400">
                    {job.order_number || job.metadata?.sales_order_number || '—'} · {job.box_type || 'Pack'}
                  </p>
                </td>
                <td className="px-5 py-4"><ProductionStageBadge stage={job.production_stage} /></td>
                <td className="px-5 py-4">{job.assigned_to || job.assigned_staff || 'Unassigned'}</td>
                ...
                <td className="px-5 py-4">
                  {linkToDetail ? (
                    <Link
                      ...
                      className="inline-block rounded-full border border-slate-700 bg-slate-900/90 px-4 py-2 text-xs font-semibold text-white transition hover:bg-slate-800"
                    >
                      Open
                    </Link>
                  ) : (
