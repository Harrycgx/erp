  ...
  const handleConvert = async () => {
    try {
      ...
      const { data, error: orderError } = await approveQuotationWorkflow({
        ...
        note: `Approved from quotation ${quote.quotation_number || quote.quote_number || quote.id}`,
      ...
      if (data?.productionError) {
        setError(`Quotation converted. Note: ${data.productionError.message || data.productionError}`);
      }
      if (data?.salesOrder?.id) {
        navigate(`/sales-orders/${data.salesOrder.id}`);
        return;
      }
      await loadQuotation();
    } catch (orderError) {
