...
/**
 * Preview format PRD-2026-0001 (DB trigger assigns authoritative number on insert).
 */
export function generateProductionNumber(sequence = 1) {
  const year = new Date().getFullYear();
  const seq = Math.max(Number(sequence) || 1, 1);
  return `PRD-${year}-${String(seq).padStart(4, '0')}`;
}

export function buildProductionNumber(sequence) {
  return generateProductionNumber(sequence);
}
