import { validateQuotationData } from '../services/quotationService';

export function validateQuotationInput(data, items = []) {
  const itemRows = items.length ? items : data.quotation_items || [];

  if (itemRows.length) {
    const { message } = validateQuotationData({ quotation: data, items: itemRows });
